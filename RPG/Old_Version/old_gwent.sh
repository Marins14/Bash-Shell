...
combatAgainstVampire() {
    ...
    sleep 2 
    echo "Vampire: Well well if isn't the $type most brave I've ever seen, let's check how powerful you are"
    sleep 2

    # Pick up the vampire attack
    vampire=$(awk -F'=' '/attack/ {print $2}' vampire.txt)

    #Compares the both attack if different (for now will be always)
